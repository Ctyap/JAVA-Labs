...
class Lab4Test {
	...
	void problem1() {
		...
		assertEquals(3, adriana.getId());
		
		// Test case 4
		Student abinav = new Student("Abinav Krishna");
		assertEquals(4, abinav.getId());
	}
	...
	void problem3() {
		...
		assertEquals(125.25, c.getTotalAmountSpent());
		c.makePurchase(9.99);
		assertEquals(0, c.getNumDiscountsToUse());
		assertEquals(134.241, c.getTotalAmountSpent(), 0.00001);
		c.makePurchase(234.30);
		assertEquals(2, c.getNumDiscountsToUse());
		assertEquals(368.541, c.getTotalAmountSpent(), 0.00001);
		c.makePurchase(10);
		assertEquals(1, c.getNumDiscountsToUse());
		assertEquals(377.541, c.getTotalAmountSpent(), 0.00001);
